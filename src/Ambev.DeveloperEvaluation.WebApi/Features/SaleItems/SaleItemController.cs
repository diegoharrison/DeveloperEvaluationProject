...
public class SaleItemController : BaseController
{
    ...
    /// <summary>
    /// Initializes a new instance of SaleItemController
    /// </summary>
    ...
    {
        var validator = new UpdateSaleItem.UpdateSaleItemValidator();
        var validationResult = await validator.ValidateAsync(request, cancellationToken);

        if (!validationResult.IsValid)
            return BadRequest(validationResult.Errors);

        var command = _mapper.Map<UpdateSaleItemCommand>((id, request));
        var response = await _mediator.Send(command, cancellationToken);
